...
const FILLER_REGEX = new RegExp(
  `(?:\\b(?:${SINGLE_WORD_FILLERS.join("|")})\\b|you\\s+know)`,
  "gi"
);
...
const FillerWordsScreen = () => {
  ...
  const clearCountdown = useCallback((resetValue: boolean = true) => {
  ...
  const handleStopAndSend = useCallback(
    async (autoSubmit = false) => {
      ...
      if (snapshot) {
        setStatusMessage(
          autoSubmit
            ? "Timer's up! Submitting your take for review..."
            : "Analyzing your take for filler words..."
        );
        ...
        setStatusMessage(
          autoSubmit
            ? "Timer ran out, but no speech detected. Try another take."
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      setStatusMessage(
        "Microphone permission was denied. Please allow access to practice."
      );
      return;
    }
  ...
  const latestMessage = useMemo(() => {
    return (
      ...
      "Give me a strong intro with no filler words. Ready when you are."
    );
  }, [messages]);
          ...
          <div className="grid gap-6 lg:grid-cols-2">
              ...
              <div className="relative overflow-hidden rounded-3xl border border-white/30 bg-white/90 p-6 shadow-2xl">
                <div className="flex flex-col items-center justify-center gap-6 text-center md:flex-row md:items-center md:justify-between md:text-left">
                  <div className="relative flex flex-col items-center justify-center gap-4">
                    <div className="relative flex h-52 w-52 items-center justify-center rounded-[36px] bg-linear-to-br from-[#111827] via-[#1F2937] to-[#0F172A] shadow-[0_25px_50px_rgba(15,23,42,0.35)] md:h-56 md:w-56">
                      <div className="absolute inset-3 rounded-[30px] bg-linear-to-br from-[#0EA5E9] via-[#2563EB] to-[#7C3AED] opacity-20 blur-sm" />
                      <span className="relative text-7xl font-black tracking-tight text-white tabular-nums">
                        {displayCountdown}
                      </span>
                      <span className="absolute bottom-6 text-xs font-semibold uppercase tracking-[0.4em] text-white/70">
                        seconds
                      </span>
                    </div>
                  ...
                  <div className="relative flex-1 space-y-4 text-[#1F2937] md:pl-8">
                    <div className="inline-flex items-center gap-2 rounded-full bg-[#FEE2E2] px-4 py-1 text-xs font-semibold uppercase tracking-widest text-[#B91C1C]">
                      {isCountdownActive ? "Live challenge" : "Ready to launch"}
                    </div>
                    <h2 className="text-3xl font-black leading-tight md:text-4xl">
                      Beat the filler words before the timer hits zero.
                    </h2>
                    <p className="text-base text-[#4B5563]">
                      Hit record and keep the clock alive. When it reaches zero we’ll stop, submit, and score your take automatically.
                    </p>
              ...
              <Card>
                <div className="grid gap-6 text-left text-[#1F2937] md:grid-cols-2">
                  <div className="space-y-4">
                    <h3 className="text-xl font-bold uppercase tracking-[0.3em] text-[#2563EB]">
                      Countdown challenge
                    </h3>
                    <p className="text-sm text-[#4B5563]">
                      Play “Coach Trent” in your head—stay locked in while the timer is on screen. Every filler word adds penalty seconds to your run.
                    </p>
                    <div className="space-y-3">
                      <div className="flex items-start gap-3 rounded-2xl border border-[#BFDBFE] bg-[#EFF6FF] p-4">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-[#1F2937]">Outline a 30-second pitch about why communication matters.</p>
                        </div>
                      </div>
                  ...
                  <div className="flex flex-col justify-between rounded-3xl border border-[#D1D5DB] bg-white/80 p-6 shadow-sm">
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-3 text-sm text-[#4B5563]">
                        <li>
                          <span className="font-semibold text-[#2563EB]">Breathe first.</span> Slow inhale, exhale, then launch into your take.
                        </li>
                        <li>
                          <span className="font-semibold text-[#2563EB]">Anchor each sentence.</span> Close one thought before jumping to the next.
                        </li>
                        <li>
                          <span className="font-semibold text-[#2563EB]">Catch yourself live.</span> If a filler word slips out, pause, reset, keep going.
                        </li>
                      </ul>
                    </div>
                    <div className="mt-6 rounded-2xl border border-[#BFDBFE] bg-[#EFF6FF] p-4 text-sm text-[#1F2937]">
                      Every second you stay clean builds muscle memory. Stack streaks,
                      then push past your longest run. Timer’s ticking.
                    </div>
                  </div>
                </div>
              </Card>
            ...
            <div className="flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                      {fillerBreakdown.length > 0 ? (
                        fillerBreakdown.map(({ word, count }) => (
                          ...
                          >
                            <span>{word}</span>
                            <span className="ml-2 rounded-full bg-[#356B47]/10 px-2 py-0.5 text-[10px]">
                              {count}
                            </span>
                          </span>
                        ))
                      ) : (
                        <span className="col-span-full text-xs text-[#4A5A52]">
                          None detected yet—keep it clean!
                        </span>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex flex-col gap-5 rounded-3xl border border-white/40 bg-white/75 px-6 py-6 text-center text-[#4A3F35] shadow-lg backdrop-blur">
                ...
                <p className="text-sm text-[#4A5A52]">
                  Hit record, let the countdown auto-submit your take, and jump to your progress report when you’re done.
                </p>
                <Toolbar>
                  <button
